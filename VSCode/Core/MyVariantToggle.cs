...
namespace TFModFortRisePickupArrowBomb
{
  ...
  public class MyVariantToggle : IHookable
  {
    ...
    private static bool IsOurs(VariantToggle toggle)
    {
      ...
      return Same(shown, Variants.TITLE) || Same(shown, Variants.ArrowBomb?.Name);
    }
    ...
    {
      TFModFortRisePickupArrowBombSettings s = TFModFortRisePickupArrowBombModule.Settings;
      ...
      return new List<VariantPopup.Field>
      {
        ...
        VariantPopup.Int("ARROWS", () => s.numberArrow, v => s.numberArrow = v, 1, 100),
        VariantPopup.Choice("ARROW TYPE",
            new[] { "Normal", "Bomb", "SuperBomb", "Laser", "Bramble", "Drill", "Bolt",
                    "Toy", "Feather", "Trigger", "Prism" },
            () => s.arrowType, v => s.arrowType = v)
      };
    }
