...
{
  /// <summary>
  /// Fait entrer la bombe dans le tirage des coffres, par la voie que FortRise
  /// prevoit pour cela.
  ///
  ...
  public class TreasureRates : IRegisterable, ITowerHook
  {
    ...
    {
      registry.TowerHooks.RegisterTowerHook("ArrowBombRates", new TreasureRates());
    }
    ...
    /// pour les pickups des mods : sans repasser derriere, la bombe tomberait alors
    /// que sa variante n'est pas cochee.
    /// </summary>
    ...
    public void VersusTowerTreasurePatch(IVersusTowerTreasurePatchContext ctx)
    {
      try
      {
        ...
        int rate = WantedRate();
        Logger.Info($"[Rates] bombe : taux {rate}");
    ...
    private static int WantedRate()
    {
      ...
      {
        Logger.Info("[Rates] la bombe : taux 0 car periodicite TEST "
      ...
      {
        Logger.Info("[Rates] la bombe : taux 0 car la variante n'est pas cochee - "
            + "la cocher, ou activer le reglage 'Pickup activated' du mod");
        return 0;
      }

      int units = Rarity.UnitsOf(TFModFortRisePickupArrowBombModule.Settings.treasureRarity);

      if (units <= 0)
      {
        Logger.Info("[Rates] la bombe : taux 0 car le cran d'apparition est au minimum");
      }
